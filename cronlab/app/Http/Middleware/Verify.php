...
class Verify
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if ($data){
            ...
            session()->flash('message', 'You are trying to use with null Purchase Code. Please purchase CronLab PTC and fill purchase code in .env file. Otherwise we will take a action for you and we will not take any liability of anything goes happen with your website or data.');
            Session::flash('type', 'error');
            Session::flash('title', 'Error: 404 | Null Detected');
            return $next($request);
        }
    }
}
